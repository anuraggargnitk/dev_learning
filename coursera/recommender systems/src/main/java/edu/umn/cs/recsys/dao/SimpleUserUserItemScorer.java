...
public class SimpleUserUserItemScorer extends AbstractItemScorer {
	...
	public void score(long user, @Nonnull MutableSparseVector scores) {
		...
		// map to cache mean-centered user vectors
		final Map<Long, MutableSparseVector> meanCenteredUserVectorsCache = new HashMap<Long, MutableSparseVector>();
		...
		for (VectorEntry e : scores.fast(VectorEntry.State.EITHER)) {
			...
			if (itemRatings != null) {
				// create a priority queue for storing 30 nearest neighbors of
				// current user who rated current item
				...
				for (Event event : itemRatings) {
					...
					if (userId != user) {
						// get the mean-centered user vector
						MutableSparseVector mutNeighborVector = getMeanCenteredUserVector(userId, meanCenteredUserVectorsCache);
						// compute the mean-centered cosine vector similarity
						// b/w
						// the current user and its possible nearest neighbor
						double similarityValue = new CosineVectorSimilarity()
								.similarity(targetUserVector, mutNeighborVector);
						...
						Map.Entry<SparseVector, Double> minSimilarityVector = nSimilarUsers
								.peek();
						// if PQ is empty or if it has elements less than
						// required neighbors
						if (minSimilarityVector == null
							...
							nSimilarUsers.add(simUsrVal);
								...
								.compare(minSimilarityVector.getValue(),
										similarityValue) == -1) {
							// PQ already has 30 elements & if the head of PQ is
							// less than current
	...
	/**
	 * This ensures that fetching
	 * <li>user vector,</li>
	 * <li>making a mutable copy,</li>
	 * <li>computing mean and</li>
	 * <li>subtracting mean from original ratings</li>
	 * is done only once during the whole course.
	 * @param userId
	 * @param meanCenteredUserVectorsCache
	 * @return
	 */
	private MutableSparseVector getMeanCenteredUserVector(long userId,
			Map<Long, MutableSparseVector> meanCenteredUserVectorsCache) {
		// lets search it from cache first
		MutableSparseVector mutableUserCenteredVector = meanCenteredUserVectorsCache
				.get(userId);

		if (mutableUserCenteredVector == null) {
			// user is being searched for the first time
			//get it from ratings.csv
			SparseVector possibleNeighborVector = getUserRatingVector(userId);
			// compute its mean
			double simUserMean = possibleNeighborVector.mean();
			// get the mutable copy of user vector so that operations can be performed on it
			mutableUserCenteredVector = possibleNeighborVector.mutableCopy();
			// subtract the mean
			mutableUserCenteredVector.add(-1 * simUserMean);
			// store it in the cache
			meanCenteredUserVectorsCache.put(userId, mutableUserCenteredVector);
		}
		return mutableUserCenteredVector;
	}
