...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
	...
	public TFIDFModel get() {
		...
		for (long item : items) {
			...
			// get number of times tag is repeated for the item
			String previousItemTag = null;
			for (String itemTag : itemTags) {
				long tagId = tagIds.get(itemTag);
				if (!itemTag.equals(previousItemTag)) {
					docFreq.add(tagId, 1d);
					work.set(tagId, 1d);
				} else {
					work.add(tagId, 1d);
				}
				previousItemTag = itemTag;
			}
		...
		for (VectorEntry e : docFreq.fast()) {
			...
			double logIDFVal = Math.log((totalItems * 1d) / e.getValue());
			docFreq.set(e.getKey(), logIDFVal);
		}
