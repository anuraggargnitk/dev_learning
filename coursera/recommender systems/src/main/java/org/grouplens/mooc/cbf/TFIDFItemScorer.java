...
public class TFIDFItemScorer extends AbstractItemScorer {
	...
	public void score(long user, @Nonnull MutableSparseVector output) {
		...
		double userVecLen = userVector.norm();
		...
		for (VectorEntry e : output.fast(VectorEntry.State.EITHER)) {
			...
			double cosineVal = (userVector.dot(iv)) / (iv.norm() * userVecLen);
			output.set(e.getKey(), cosineVal);
		}
	}
	...
	private SparseVector makeUserVector(long user) {
		...
		for (Rating r : userRatings) {
			...
			/*
			 * if (p != null && p.getValue() >= 3.5) { // The user likes this
			 * item! // TODO Get the item's vector and add it to the user's
			 * profile SparseVector itemVector =
			 * model.getItemVector(p.getItemId()); profile.add(itemVector); }
			 */
			meanVal = meanVal + p.getValue();
		}
		meanVal = meanVal / userRatings.size();
		//part2 - weighted (userrating - mean) * itemProfile
		for (Rating r : userRatings) {
			Preference p = r.getPreference();
			SparseVector itemVector = model.getItemVector(p.getItemId());
			MutableSparseVector itmVecMutable = itemVector.mutableCopy();
			itmVecMutable.multiply(p.getValue() - meanVal);
			profile.add(itmVecMutable);
		}
